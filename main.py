...
def helper(dis):
    ...
    # Retrieve disease precautions
    pre = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pre = [col for col in pre.values]

    # Retrieve disease medications (ensuring it's a list)
    med = medications[medications['Disease'] == dis]['Medication']
    med = med.values.tolist()  # Convert to list

    # Retrieve disease diet (ensure it's a list)
    die = diets[diets['Disease'] == dis]['Diet']
    die = die.values.tolist()

    # Retrieve disease workouts (ensure it's a list)
...
def get_predicted_value(patient_symptoms):
    ...
    # Normalize input symptoms to lowercase and remove extra spaces
    normalized_symptoms = [symptom.strip().lower() for symptom in patient_symptoms]
    
    for symptom in normalized_symptoms:
        # Find the closest match in symptoms_dict
        best_match = process.extractOne(symptom, symptoms_dict.keys())
        
        # If a match is found with a high enough score, use it
        if best_match and best_match[1] >= 80:  # You can adjust the threshold (80) for how close the match should be
            matched_symptom = best_match[0]
            input_vector[symptoms_dict[matched_symptom]] = 1
        else:
            print(f"Warning: '{symptom}' does not match any known symptom closely.")
    
    predicted_value = svc.predict([input_vector])
    return diseases_list.get(predicted_value[0], "Unknown Disease")
